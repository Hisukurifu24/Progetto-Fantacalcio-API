...
public class TeamService {
    ...
    public TeamResponse create(CreateTeamRequest request) {
        ...
        if (!league.getMembers().contains(currentUser)) {
             // Logic depends on requirements. For now, let's assume if you can create a team, you must be in the league.
             ...
             // I'll add a check for safety if I can, but strict adherence might block if logic is different.
    ...
    public List<TeamResponse> findByLeague(UUID leagueId) {
        ...
        // Using stream for now to avoid modifying repository interface in this step if not strictly needed, 
        ...
        // I will update the repo in a separate tool call if needed or just rely on stream if dataset is small.
